...
<!-- Modal -->
<div class="modal fade" id="editMarksModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
        ...
        $(document).ready(function(){
            ...
            $('#pass_marks').keyup(function(){
                ...
                if(parseFloat(pmarks) >=parseFloat(tmarks)){
                    $(this).parent().append('<p style="color:red;" class="pass-error">passing marks will be less then total marks</p>')
                    setTimeout(()=>{
                        $('.pass-error').remove();
                    },2000);
                }
            ...
            $('.pass-error').remove();
            if(parseFloat(pmarks) >=parseFloat(tmarks)){
                $('#pass_marks').parent().append('<p style="color:red;" class="pass-error">passing marks will be less then total marks</p>')
                setTimeout(()=>{
                    $('.pass-error').remove();
                },2000);
                return false;
