...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
  ...
  if (empty($fname_err) && empty($lname_err) && empty($email_err) && empty($course_err) && empty($batch_err) && empty($city_err) && empty($state_err)) {
    ...
    $sql = "UPDATE teachers SET firstname = ?, lastname = ?, email = ?, course = ?, batch = ?, city = ?, state = ? WHERE id = ? ";
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        echo "<script>window.location.href='http://localhost/php_crud/teacher.php';</script>";
        exit;
      } else {
  ...
  <div class="container" style="background-color:powderblue;">
    <div class="row justify-content-center pt-5">
      <div class="col-lg-6">
        ...
        <h4 align="center">Teacher info update
        <p class="text-end">
      <a href="teacher.php" class="btn btn-secondary">&laquo; Go back</a>
    </p>
      </h4>
        </div>
          <div class="row">
